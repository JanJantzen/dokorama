...
- [x] 10.2 Spieler-Steckbrief-Gerüst. — *erledigt & abgenommen 05.08.2026.* NEU: `rankMap(werte, higherIsBetter)` in `lib/stats.js` (Ränge übers ganze Feld, Gleichstand = gleicher Rang + Lücke; Aufrufer siebt null/schwach aus). In `StatsPage.jsx`: Kennzahl-Registry `PROFILE_METRICS` (9 Leitwerte L6/L1/L9/L2/L5/L7 + A1/A3/A6, ebenen-abhängige auf Partie, schwache Stichproben ohne Rang) + `buildProfile(data, playerId)` (rechnet auf `periodFiltered` → Ränge übers ganze Feld, Personen-Filter außen vor). `PlayerProfile.jsx` rendert B (Gesamt/Schnitt je mit 🥇🥈🥉/#n) + C (Meine Werte: Standard = Top-3-Platzierungen, Fallback = 3 beste Ränge, „Alle Werte anzeigen" = **eine wachsende Liste**, keine Doppelung). **Datenqualität:** Registry-Kennzahlen können ein `note` (String oder `(data)=>string`) tragen → Fußnote mit fortlaufender Ziffer; Spielzeit (A6) blendet das früheste App-Datum im Zeitraum ein (P2). **Entscheidungen (Jan, 03.08.2026):**
